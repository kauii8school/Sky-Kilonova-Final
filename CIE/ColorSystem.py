...
import numpy as np
import sys
from ColorMatchingFunc import *
import os
cwd = os.getcwd()
CIEPath = os.path.join(cwd, "CIE")
# sys.path.insert(0, )
...
class ColourSystem:
    ...
    def cmfCreation(self, wavelengthList):
        temp = []
        for wav in wavelengthList:
            temp.append(np.array((xBar(wav), yBar(wav), zBar(wav))))

        self.cmf = np.array(temp)
        return self.cmf
